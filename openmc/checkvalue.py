...
def check_type(name, value, expected_type, expected_iter_type=None, *, none_ok=False):
    ...
    if not isinstance(value, expected_type):
        if isinstance(expected_type, Iterable):
            msg = 'Unable to set "{}" to "{}" which is not one of the following types: "{}"'.format(
                name, value, ", ".join([t.__name__ for t in expected_type])
            )
        else:
            msg = f'Unable to set "{name}" to "{value}" which is not of type "{expected_type}"'
        raise TypeError(msg)
                ...
                if isinstance(expected_iter_type, Iterable):
                    msg = (
                        ...
                        'one of the following types: "{}"'.format(
                            name, value, ", ".join([t.__name__ for t in expected_iter_type])
                        )
                    )
                else:
                    msg = (
...
def check_length(name, value, length_min, length_max=None):
    ...
    if length_max is None:
        ...
        if length_min == length_max:
            msg = f'Unable to set "{name}" to "{value}" since it must be of length "{length_min}"'
        else:
            msg = (
...
def check_increasing(name: str, value, equality: bool = False):
    ...
    if equality:
        if not np.all(np.diff(value) >= 0.0):
            raise ValueError(
                f'Unable to set "{name}" to "{value}" since its elements must be increasing.'
            )
    elif not equality:
        if not np.all(np.diff(value) > 0.0):
            raise ValueError(
...
def check_value(name, value, accepted_values):
    ...
    if value not in accepted_values:
        msg = f'Unable to set "{name}" to "{value}" since it is not in "{accepted_values}"'
        raise ValueError(msg)
...
def check_less_than(name, value, maximum, equality=False):
    ...
    if equality:
        if value > maximum:
            msg = f'Unable to set "{name}" to "{value}" since it is greater than "{maximum}"'
            raise ValueError(msg)
    else:
        if value >= maximum:
            msg = (
...
def check_greater_than(name, value, minimum, equality=False):
    ...
    if equality:
        if value < minimum:
            msg = f'Unable to set "{name}" to "{value}" since it is less than "{minimum}"'
            raise ValueError(msg)
    else:
        if value <= minimum:
            msg = (
                f'Unable to set "{name}" to "{value}" since it is less than or equal to "{minimum}"'
            )
            raise ValueError(msg)
        ...
        if this_version[0] != expected_version:
            raise OSError(
                ...
                "consistent with the version expected by OpenMC, {}".format(
                    this_filetype, ".".join(str(v) for v in this_version), expected_version
                )
            )
